/*
https://www.codewars.com/kata/59bf97cd4f98a8b1cd00007e/train/javascript
...
function solve(a,b){
    ...
    for (let i = a; i < b; i++) {
        ...
        let ratio = 0;
        for (let j = 1; j <= i; j++) {
            if (i / j === Math.round(i / j)) sum += j
        }
        ratio = sum / i;
        ratio in obj ? obj[ratio].push(i) : obj[ratio] = [i];
    }
    let result = 0;
    for (key in obj) {
        if (obj[key].length > 1) result += obj[key][0];
    }
    return result;
};